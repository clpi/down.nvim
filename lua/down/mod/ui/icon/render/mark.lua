...
function M.is_same_line_movement(event)
...
M.handlers = {
  ["cmd.events.icon.toggle"] = M.on.toggle.pretty,
}
M.on = {
  ft = function(event)
    M.on.event(event)
    ...
    moved = function(event)
      if not M.is_same_line_movement(event) then
        ...
        if cursor_record then
          M.ln.mark.changed(event.buffer, cursor_record.row_0b)
        end
        local current_row_0b = event.cursor_position[1] - 1
        M.ln.mark.changed(event.buffer, current_row_0b)
      end
      M.on.cursor.update(event)
    end,

    moved_i = function(event)
      return M.on.cursor.moved(event)
    end,
  },
  event = function(event)
    ...
    local function on_line_callback(
      ...
      M.ln.mark.changed(bufid, row_start_0b, row_updated_0bex)
    end
    ...
    local function on_changedtree_callback(ranges)
      ...
      for i = 1, #ranges do
        ...
        M.range.pretty.rm(bufid, row_start_0b, row_end_0bex)
      end
    end

    language_tree:register_cbs({ on_changedtree = on_changedtree_callback })
    M.all.mark.changed(event.buffer)
  ...
  insert = {
    ...
    toggle = function(event)
      M.ln.mark.changed(event.buffer, event.cursor_position[1] - 1)
    end,

    enter = function(event)
      M.on.insert.toggle(event)
    end,
...
function M.inside_example(_)
  -- TODO: waiting for parser fix
  return false
end

M.skip_prettify = function(
  current_mode,
  current_row_0b,
  node,
  config,
  ...
  then
    result = true
  elseif M.inside_example(node) then
    result = true
  else
    result = false
  end
...
M.opts = function(row_0b, col_0b, text, highlight)
  return {
...
M.clear = function(bufid)
  vim.api.nvim_buf_clear_namespace(bufid or 0, M.ns.icon.ns, 0, -1)
  vim.api.nvim_buf_clear_namespace(bufid or 0, M.ns.pretty.ns, 0, -1)
end

M.set = function(bufid, row_0b, col_0b, text, highlight, ext_opts)
  local opt = M.opts(row_0b, col_0b, text, highlight)

  if ext_opts then
    tbl.extendinplace(opt, ext_opts)
  end

  vim.api.nvim_buf_set_extmark(bufid, M.ns.icon.ns, row_0b, col_0b, opt)
end
M.remove = function(bufid, pos_start_0b_0b, pos_end_0bin_0bex)
  if pos.eq(pos_start_0b_0b, pos_end_0bin_0bex) then
    return
  end

  local ns_icon = M.ns.icon.ns
  for _, result in
    ipairs(
  ...
  do
    ...
    --     )
    -- )
    vim.api.nvim_buf_del_extmark(bufid, M.ns.icon.ns, extmark_id)
  end
end
...
M.ln = {
  ...
  schedule = function(bufid)
    vim.schedule(function()
      vim.api.nvim_buf_clear_namespace(bufid, M.ns.pretty.ns, 0, -1)
    end)
  end,
  pretty = {
    rm = function(bufid, row_0b)
      -- TODO: optimize
      local ns_prettify_flag = M.ns.pretty.ns
    ...
    add = function(bufid, row)
      vim.api.nvim_buf_set_extmark(bufid, M.ns.pretty.ns, row, 0, {})
    end,
  },
}

function M.get_parsed_query_lazy(p)
  if p then
    return p
  end
...
M.range = {
  ...
  pretty = {
    ...
    render = function(bufid, row_start_0b, row_end_0bex, p)
      ...
      local nodes, concealed_node_ids = M.query_get_nodes(
        M.get_parsed_query_lazy(p),
        document_root,
        bufid,
        row_start_0b,
        row_end_0bex
      ...
      for _, node in ipairs(nodes) do
        ...
        if config.clear then
          ...
          M.remove(bufid, pos_start_0b_0b, pos_end_0bin_0bex)
        end

        M.range.pretty.rm(node_row_start_0b, node_row_end_0bex)
        M.range.pretty.add(bufid, node_row_start_0b, node_row_end_0bex)
...
--- @param bufid? integer
M.render_buf = function(bufid, p)
  bufid = bufid or vim.api.nvim_get_current_buf()
  local ns_prettify_flag = M.ns.pretty.ns
  local winid = vim.fn.bufwinid(bufid)
  local row_start_0b, row_end_0bex = M.ln.visible(winid)
...
M.all = {
  render = function()
    for bufid, _ in pairs(M.rerendering_scheduled_bufids) do
      if vim.fn.bufwinid(bufid) >= 0 then
        M.render_buf(bufid, M.prettify_query)
      end
    end
  ...
  mark = {
    ...
    changed = function(bufid)
      ...
      M.all.pretty.rm(bufid)
      M.schedule(bufid)
    end,
  },
  pretty = {
    rm = function(bufid)
      M.range.pretty.rm(bufid, 0, -1)
    end,
  },
}
