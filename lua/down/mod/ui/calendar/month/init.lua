...
M.setup_view = function(ui_info, mode, date, options)
  options.distance = options.distance or 4
  local view = M.new_view_instance()
  ...
  do
    ...
    vim.keymap.set('n', 'i', function()
      ...
      vim.keymap.set({ 'n', 'i' }, '<CR>', function()
        ...
        local parsed_date = M.dep['data.time'].parse_date(line)
        ...
        local lua_date = M.dep['data.time'].to_lua_date(parsed_date)
...
M.set_extmark = function(ui_info, namespace, row, col, length, virt_text, alignment, extra)
  if alignment then
    local text_length = 0
...
M.set_decorational_extmark = function(ui_info, row, col, length, virt_text, alignment, extra)
  return M.set_extmark(
    ui_info,
    M.namespaces.decorational,
    row,
    col,
    length,
    virt_text,
...
M.set_logical_extmark = function(ui_info, row, col, virt_text, alignment, extra)
  return M.set_extmark(ui_info, M.namespaces.logical, row, col, nil, virt_text, alignment, extra)
end
...
M.new_view_instance = function()
  return {
    ...
    render_month_banner = function(self, ui_info, date, weekday_banner_extmark_id)
      ...
      self.extmarks.decorational.month_headings[weekday_banner_extmark_id] = M.set_decorational_extmark(
        ui_info,
        4,
        weekday_banner_id[2]
    ...
    render_weekday_banner = function(self, ui_info, offset, distance)
      ...
      local weekday_banner_id = M.set_decorational_extmark(
        ui_info,
        6,
        extmark_position,
    ...
    render_month = function(self, ui_info, target_date, weekday_banner_extmark_id)
      ...
      for day_of_month, day_of_week in ipairs(days_of_month) do
        ...
        self.extmarks.logical.months[month][day_of_month] =
          vim.api.nvim_buf_set_extmark(ui_info.buffer, M.namespaces.logical, start_row, start_col, {
            virt_text = {
              {
    ...
    render_month_array = function(self, ui_info, date, options)
      ...
      local months_to_render = M.rendered_months_in_width(ui_info.width, options.distance)
    ...
    select_current_day = function(self, ui_info, date)
      ...
      local position =
        vim.api.nvim_buf_get_extmark_by_id(ui_info.buffer, M.namespaces.logical, extmark_id, {})
    ...
    render_view = function(self, ui_info, date, previous_date, options)
      ...
      if is_first_render then
        vim.api.nvim_buf_clear_namespace(ui_info.buffer, M.namespaces.decorational, 0, -1)
        vim.api.nvim_buf_clear_namespace(ui_info.buffer, M.namespaces.logical, 0, -1)
    ...
    clear_extmarks = function(self, ui_info, current_date, options)
      ...
      local rendered_months_offset =
        math.floor(M.rendered_months_in_width(ui_info.width, options.distance) / 2)
      ...
      local clear_extmarks_for_month = function(month)
        for _, extmark_id in ipairs(self.extmarks.logical.months[month]) do
          vim.api.nvim_buf_del_extmark(ui_info.buffer, M.namespaces.logical, extmark_id)
        end
...
M.load = function()
  mod.get('ui.calendar').add_view(M.view_name, M)
end
