...
M.setup_view = function(ui_info, mode, date, options)
  ...
  do
    ...
    vim.keymap.set("n", "g", function()
      local day =
        math.min(vim.v.count1, M.get_month_length(date.month, date.year))
...
M.set_decorational_extmark = function(
  ui_info,
  row,
  col,
  length,
  ...
  return {
    ...
    render_month_array = function(self, ui_info, date, options)
      ...
      local months_to_render =
        M.rendered_months_in_width(ui_info.width, options.distance)
    ...
    render_decorative_text = function(self, ui_info, view)
      ...
      self.extmarks.decorational =
        vim.tbl_deep_extend("force", self.extmarks.decorational, {
          ...
          help_and_custom_input = M.set_decorational_extmark(
            ui_info,
            ui_info.height - 1,
            0,
            0,
          ...
          current_view = M.set_decorational_extmark(
            ui_info,
            ui_info.height - 1,
            0,
            0,
    ...
    clear_extmarks = function(self, ui_info, current_date, options)
      ...
      local rendered_months_offset = math.floor(
        M.rendered_months_in_width(ui_info.width, options.distance) / 2
      )
