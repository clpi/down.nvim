...
M.shortcut = [[ (shortcut_link) @shortcut ]]
M.paragraph = [[ (section (paragraph)) @paragraph ]]
M.link_query = [[
                (link) @next-segment
...
M.load = function()
  ...
  if M.config.configure_parsers then
    ...
    --   once = true,
    --   callback = function()
    --     M.parser_path = vim.api.nvim_get_runtime_file("parser/down.so", false)[1]
    --
    --     if M.parser_path then
    --       return
    --     end
    --
...
M.data = { os_utils = tsu }
M.parse = function(language, query)
...
M.inject = function(language, injection)
  ...
  for id, node in next_match_query:iter_captures(document_root, 0, line_number - 1, -1) do
    if next_match_query.captures[id] == "next-segment" then
      ...
      if (start_line == line_number and start_col > col_number) or start_line > line_number then
        M.os_utils.goto_node(node) ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
        return
      end
    end
...
---@param query_string string Query with `@next-segment` captures
M.goto_previous_query_match = function(query_string)
  local cursor = vim.api.nvim_win_get_cursor(0)
  local line_number, col_number = cursor[1], cursor[2]
  ...
  if final_node then
    M.os_utils.goto_node(final_node) ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
  end
end
...
M.get_all_nodes = function(node_type, opts)
  local result = {}
  opts = opts or {}
  ...
  ts.get_parser(opts.buf, opts.ft):for_each_tree(function(tree)
    table.insert(result, M.search_tree(tree, node_type))
  end)
...
M.get_all_nodes_in_file = function(node_type, path, filetype)
  ...
  return M.search_tree(tree, node_type)
end
M.fix_indent = function(text)
  local _l = "";
...
M.search_tree = function(tree, node_type)
  local result = {}
...
M.tree_map = function(callback, ts_tree)
...
---@param ts_tree any #Optional syntax tree ---@diagnostic disable-line -- TODO: type error workaround <pysan3>
M.tree_map_rec = function(callback, ts_tree)
  local tree = ts_tree or ts.get_parser(0, "markdown"):parse()[1]
...
M.swap_nodes = function(node1, node2, bufnr, cursor_to_second)
  if not node1 or not node2 then
    return
  end
  local range1 = M.node_to_lsp_range(node1)
  local range2 = M.node_to_lsp_range(node2)

  local text1 = M.get_node_text(node1, bufnr)
  local text2 = M.get_node_text(node2, bufnr)
...
M.get_first_node = function(type, buf, parent)
  if not buf then
    buf = 0
  end
...
M.get_tag_info = function(tag_node, throw)
  if
      not tag_node
    ...
    if vim.endswith(child:type(), "_carryover_set") then
      for subchild in child:iter_children() do
        if vim.endswith(subchild:type(), "_carryover") then
          local meta = M.get_tag_info(subchild, throw)
...
M.get_first_node_on_line = function(buf, line, stop_type)
  ...
  local document_root = M.get_document_root(buf)
...
M.get_document_metadata = function(source, no_trim)
  ...
  local down_parser, iter_src = M.get_ts_parser(source)
  ...
  local function parse_data(node, src)
    return lib.match(node:type())({
      ...
      number = function()
        return tonumber(M.get_node_text(node, src))
      end,
      array = function()
      ...
      object = function()
        ...
        for child in node:iter_children() do
          ...
          local key_content = trim(M.get_node_text(key, src))
  ...
  for id, node in down_query:iter_captures(down_tree:root(), iter_src) do
    if down_query.captures[id] == "tag_name" then
      local tag_name = trim(M.get_node_text(node, iter_src))
  ...
  local meta_source = M.get_node_text(meta_node, iter_src)
  ...
  for id, node in meta_query:iter_captures(markdown_inline_tree:root(), meta_source) do
    if meta_query.captures[id] == "key" then
      local key = trim(M.get_node_text(node, meta_source))

      local val
      if key == "title" then
        -- force title's value as string type
        val = trim(M.get_node_text(node:next_named_sibling(), meta_source))
      else
