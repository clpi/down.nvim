...
--- Find and apply/edit templates in workspace
---@param opts table
return function (opts)
  ...
  local template_dirs = {
    vim.fs.joinpath (ws_path, ".down", "templates"),
    ...
    .new (opts, {
      ...
      attach_mappings = function (prompt_bufnr, map)
        ...
        actions.select_default:replace (function ()
          ...
          if selection then
            local tmpl_mod = mod.get_mod ("template")
            if tmpl_mod then
              vim.ui.input ({ prompt = "Title: " }, function (title)
                tmpl_mod.apply_to_buf (selection.name, {
                  title = title or selection.name,
                })
              end)
            else
