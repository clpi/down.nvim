...
---@field action string Action: "index", "compact", "tag", "notify", "run"
...
local function execute (rule, file)
  ...
  elseif rule.action == "notify" then
    vim.notify ("Automate: " .. rule.name .. " triggered for " .. file, vim.log.levels.INFO)
  ...
  for name, path in pairs (ws.data.workspaces or {}) do
    if vim.fn.isdirectory (path) == 1 then
      ...
      for _, file in ipairs (files) do
        ...
        local prev = Automate.dep["data"] and Automate.dep["data"].get (key) or 0
...
function Automate.start ()
  if Automate.timer then return end
  Automate.timer = vim.fn.timer_start (Automate.config.interval, function ()
    pcall (Automate.scan)
  end, { ["repeat"] = -1 })
  log.info ("Automate: started watching (interval: " .. Automate.config.interval .. "ms)")
end
...
Automate.setup = function ()
  -- Default rules
  Automate.add { name = "auto-index", trigger = "modify", pattern = "*.md", action = "index" }
  Automate.add { name = "auto-compact", trigger = "modify", pattern = "*.md", action = "compact" }
...
Automate.commands = {
  automate = {
    ...
    name = "automate",
    callback = function (_)
      vim.notify ("Automate: " .. #Automate.rules .. " rules active", vim.log.levels.INFO)
    end,
    commands = {
      start = {
        enabled = true, args = 0, name = "automate.start",
        callback = function () Automate.start () end,
      },
      stop = {
        enabled = true, args = 0, name = "automate.stop",
        callback = function () Automate.stop () end,
      },
      add = {
        enabled = true, args = 0, name = "automate.add",
        callback = function ()
          vim.ui.input ({ prompt = "Rule name: " }, function (name)
            if name then
              vim.ui.input ({ prompt = "Pattern (glob): " }, function (pat)
                if pat then
                  vim.ui.select ({ "modify", "create", "delete" }, { prompt = "Trigger:" }, function (trig)
                    if trig then
                      vim.ui.select ({ "index", "tag", "notify", "compact" }, { prompt = "Action:" }, function (act)
                        if act then
                          Automate.add { name = name, trigger = trig, pattern = pat, action = act }
                          vim.notify ("Rule added: " .. name, vim.log.levels.INFO)
                        end
                      end)
                    end
                  end)
      ...
      list = {
        enabled = true, args = 0, name = "automate.list",
        callback = function ()
          local lines = { "# Automation Rules", "" }
          for _, r in ipairs (Automate.rules) do
            local status = r.enabled and "[x]" or "[ ]"
            lines[#lines + 1] = "- " .. status .. " " .. r.name .. " | " .. r.trigger .. " | " .. r.pattern .. " -> " .. r.action
          end
